...
class TrainingDaoTest {
    ...
    private lateinit var testSubscriber : TestSubscriber<List<Training>>
        ...
        fun insertAndGetSeriesById(){
           ...
           testDb.seriesDao().getSeriesById(series.id).test().assertValue(series)//{it->it.reps==(series.reps)}
    ...
    fun insertAndGetTrainigsForWeek(){
        ...
        fun shouldgetSeriesForTrainig(){
            ...
            testDb.trainingDao().insertTraining(training2)
            val series = Series(2, "Bench", 5, 123.5, 120, true,3)
            val series2 = Series(3, "Bench", 6, 150.0, 120, true, 3)
            val series3 = Series(4, "Bench", 7, 150.0, 120, true, 3)
