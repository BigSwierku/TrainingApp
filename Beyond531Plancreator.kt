        ...
        when (training.week)  {
            1,4 -> {
                for (i in 1..3) {
                    seriesInExcersise.add(Series(excersise = excersiseList[0],
                            reps = 3,
                            weight = (0.6 + (i* 0.1)) * (UserProperities.instance.workingMaxWeights?.getValue(excersiseList[0]) ?:0.0),
                            pause = 120,
                ...
                for (i in 1..5) {
                    seriesInExcersise.add(Series(excersise = excersiseList[1],
                            ...
                            weight = 0.9 * (UserProperities.instance.workingMaxWeights?.getValue(excersiseList[0]) ?:0.0),
                            pause = 120,
                            trainingId = training.id))
                }
            }
            ...
            2,5 -> {
                for (i in 1..3) {
                    seriesInExcersise.add(Series(excersise = excersiseList[0],
                            reps = 5,
                            weight = (0.55 + (i*0.1)) * (UserProperities.instance.workingMaxWeights?.getValue(excersiseList[0]) ?:0.0),
                            pause = 120,
                            trainingId = training.id))
                }
                for (i in 1..5) {
            ...
            3,6->{
                ...
                seriesInExcersise.add(Series(excersise = excersiseList[0],
                        reps = i,
                        weight = (1.0 - (i*0.05)) * (UserProperities.instance.workingMaxWeights?.getValue(excersiseList[0]) ?:0.0),
                        pause = 120,
                        trainingId = training.id))
            }
                for (i in 1..5) {
